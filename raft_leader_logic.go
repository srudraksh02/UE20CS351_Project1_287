	...
	for _, peerId := range this.peersIds { // Peers are other nodes.
		...
		go func(peerId int) {
			...
			if err := this.server.SendRPCCallTo(peerId, "RaftNode.AppendEntries", args, &reply); err == nil {
				...
				if reply.Term > termWhenHeartbeatSent {
					this.write_log("term out of date in heartbeat reply")
				...
				if this.state == "Leader" && termWhenHeartbeatSent == reply.Term {
					if reply.Success {
						...
						this.write_log("AppendEntries reply from %d success: nextIndex := %v, matchIndex := %v", peerId, this.nextIndex, this.matchIndex)
